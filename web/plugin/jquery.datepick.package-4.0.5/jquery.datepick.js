...
$(function(){
	...
	$("[plugin='date'][start=''][end='']").datepick({dateFormat: 'yyyy-mm-dd'});
	$("[plugin='date'][start!=''],[plugin='date'][end!='']").datepick({
    	onSelect: customRange, showTrigger: '#calImg'
	}); 
});
function customRange(dates) { 
    if ($(this).attr("start")) { 
        $("[plugin='date'][end='"+$(this).attr("start")+"']").datepick('option', 'minDate', dates[0] || null); 
    } 
    else { 
        $("[plugin='date'][start='"+$(this).attr("end")+"']").datepick('option', 'maxDate', dates[0] || null); 
    } 
}
