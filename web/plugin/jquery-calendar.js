...
var popUpCal = {
	...
	yearRange: '-8:+8', // Range of years to display in drop-down, either relative to current year (-nn:+nn) or absolute (nnnn:nnnn)
	...
	speed: 120, // Speed of display/closure
...
$(function(){
	/* 自动对有属性 plugin="date" 的对象增加控件*/
	$("[plugin='date2'][start=''][end='']").calendar();
	$("[plugin='date2'][start!=''],[plugin='date2'][end!='']").calendar({
    	onSelect: customRange, showTrigger: '#calImg'
	}); 
});
function customRange(dates) { 
    if ($(this).attr("start")) { 
        $("[plugin='date2'][end='"+$(this).attr("start")+"']").calendar('option', 'minDate', dates[0] || null); 
    } 
    else { 
        $("[plugin='date2'][start='"+$(this).attr("end")+"']").calendar('option', 'maxDate', dates[0] || null); 
    } 
}
