...
function openWindow(url,x,y){
	x=x==null?"800":x;
	y=y==null?"600":y;
	window.open(url,"Փт԰ࠚͭݓ","scrollbars=yes,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,top="+((screen.height-y)/2)+",left="+((screen.width-x)/2)+",width="+x+",height="+y);
}
