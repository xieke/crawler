...
//刷新子Select中的数据
function refreshChild(parentSelect,childId) {
//	if (parentSelect.childID == null || parentSelect.childID == '') {
	...
	//alert(parentSelect.mainSelectID);
	if (parentSelect.mainSelectID == null || parentSelect.mainSelectID =="") {//主Select刷新时

		allParament = "<root><data><"+parentSelect.name+" number=\"0\">"
		...
		while (tempSel != parentSelect) {//直到当前的Select才退出
			allParament = allParament + "<"+tempSel.name+" number=\""+j+"\">"
										+"<"+tempSel.textField+">"
		...
		allParament = allParament + "<"+ parentSelect.name+" number=\""+j+"\">"
									+"<"+parentSelect.textField+">"
									+conversion(parentSelect.options[parentSelect.selectedIndex].text)
									+"</"+parentSelect.textField+">"
									+"<"+parentSelect.id+">"
	...
	//alert("allParament is  "+allParament);
	//alert("bizAction is "+child.bizAction);
